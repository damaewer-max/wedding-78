...
<div id="invite" class="hidden">
  <div class="card">
    ...
    <div class="message">
      <p>
      Мы счастливы поделиться с вами самым важным и тёплым событием в нашей жизни — днём, когда мы станем семьёй.
      В этот особенный момент нам хочется видеть рядом тех, кто дорог нашему сердцу,
      кто разделяет с нами радость, улыбки и искренние эмоции.
      </p>

      <p>
      Приглашаем вас провести этот день вместе с нами, наполнить его смехом, объятиями,
      добрыми словами и незабываемыми воспоминаниями.
      ...
      Будем рады любому подарку. И не важно, в какой конверт вы его упакуете.
      </p>
      ...
      <p>
      Будем с нетерпением ждать вас 23 мая в 18:00 по адресу г.Ростов-на-Дону, ул. Левобережная 50, ресторан "Жардин".
      </p>
    </div>
    ...
    <button class="musicBtn" onclick="toggleMusic()">Музыка вкл/выкл</button>
...
/* экспорт Excel (UTF-8 + BOM + столбцы) */
function downloadCSV(){
  const data = JSON.parse(localStorage.getItem("wedding")) || [];

  const rows = [
    ["Имя","Количество гостей","Ответ","Время"]
  ];
